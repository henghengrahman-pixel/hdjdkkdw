...
# Semua data runtime diarahkan ke persistent DATA_DIR.
# Di Railway set Volume mount path ke /data dan DATA_DIR=/data.
_default_data_dir = "/data" if os.environ.get("RAILWAY_ENVIRONMENT") else str(BASE_DIR / "runtime-data")
DATA_DIR = Path(os.environ.get("DATA_DIR", _default_data_dir)).expanduser().resolve()
DATA_FILE = Path(os.environ.get("DATA_FILE", str(DATA_DIR / "bot_data.json"))).expanduser().resolve()
MEDIA_DIR = Path(os.environ.get("MEDIA_DIR", str(DATA_DIR / "media"))).expanduser().resolve()
BACKUP_DIR = Path(os.environ.get("BACKUP_DIR", str(DATA_DIR / "backups"))).expanduser().resolve()
LEGACY_DATA_FILE = BASE_DIR / "data.json"
LEGACY_BOT_DATA_FILE = BASE_DIR / "bot_data.json"

for _dir in (DATA_DIR, MEDIA_DIR, BACKUP_DIR):
...
def load_data():
    # Urutan: persistent file -> bot_data lama -> data.json lama.
    candidates = (DATA_FILE, LEGACY_BOT_DATA_FILE, LEGACY_DATA_FILE)
    ...
    try:
        ...
        if source != DATA_FILE:
            log.info("Migrasi data lama %s -> %s", source, DATA_FILE)
        return data
    except Exception:
        log.exception("Gagal membaca data dari %s; menggunakan default", source)
        return DEFAULT_DATA.copy()
...
def backup_data_file():
    ...
    try:
        ...
        target = BACKUP_DIR / f"bot_data-{stamp}.json"
        shutil.copy2(DATA_FILE, target)
        backups = sorted(BACKUP_DIR.glob("bot_data-*.json"), key=lambda x: x.stat().st_mtime, reverse=True)
        for old in backups[20:]:
            old.unlink(missing_ok=True)
    except Exception:
        log.exception("Backup data gagal")
...
async def send_custom(group):
    try:
        with data_lock:
            ...
            media_path = bot_data.get("media_path", "")
            caption_raw = bot_data.get("caption", "")
        buttons = build_buttons()
        caption = bold(caption_raw)

        local_media = Path(media_path) if media_path else None
        if local_media and local_media.is_file():
            ...
            if not msg or not msg.media:
                raise ValueError("Media persistent dan Saved Messages tidak ditemukan")
            await client.send_file(
                group,
                msg.media,
...
def health():
    return jsonify({
        "ok": True,
        "telegram": bool(client.is_connected()),
...
def upload_media_web():
    ...
    try:
        unique_name = f"{int(time.time())}-{secrets.token_hex(4)}{suffix}"
        persistent_path = MEDIA_DIR / unique_name
        file.save(str(persistent_path))
        if not persistent_path.is_file() or persistent_path.stat().st_size <= 0:
            raise ValueError("File media gagal disimpan ke persistent storage")

        with data_lock:
            caption = bot_data.get("caption", "")
            old_media_path = bot_data.get("media_path", "")

        # Salinan Saved Messages menjadi fallback kedua bila volume bermasalah.
        msg_id = run_async(upload_to_saved_messages(str(persistent_path), caption), timeout=120)
        ...
        with data_lock:
            ...
            bot_data["media_path"] = str(persistent_path)
            bot_data["forward_link"] = None
            save_data(bot_data)

        # Hapus file media lokal lama setelah konfigurasi baru sukses tersimpan.
        if old_media_path and old_media_path != str(persistent_path):
            try:
                old_path = Path(old_media_path)
                if old_path.is_file() and MEDIA_DIR in old_path.parents:
                    old_path.unlink()
            except Exception:
                log.warning("Tidak bisa menghapus media lama: %s", old_media_path)

        flash("Media berhasil disimpan permanen di data directory dan Saved Messages.", "success")
    except Exception as e:
...
def delete_media_web():
    ...
    if old_media_path:
        try:
            ...
            log.warning("Tidak bisa menghapus file media: %s", old_media_path)
